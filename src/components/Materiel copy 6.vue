<template>
  <div class="container my-4">
    ...
    <v-expansion-panels class="pa-7 fontPolice">
      <v-expansion-panel>
        <v-expansion-panel-title style="font-size: 20px;background:linear-gradient(200deg,#cfe4e421,#e4e4e465,#13626d23);">{{ modeEdition ? 'Modifier' : 'Nouveau' }} Matériel</v-expansion-panel-title>
    ...
    <!-- importation fichier excel matériel -->
    ...
    <div class="card shadow-lg border-0 rounded-4 p-4 mb-4 bg-light bg-opacity-75">
      ...
      <div class="pa-3">
        ...
        <h4 class="fw-bold text-primary mb-0">Importation materiels</h4>
...
<template v-slot:top>
    <v-toolbar class="pa-2" style="background:linear-gradient(250deg,#cfe4e421,#e4e4e465,#13626d17);">
      ...
      <v-text-field class="pa-5 fontPolice"
          color="primary"
          hide-details
          v-model="search"
...
try{
      ...
      if(existe){
        throw new Error("Ce code existe déja. Veuillez choisir un autre!");
      }
      ...
      if (navigator.onLine) await syncToSupabase();
        ...
        icon:'error',
        title:'Erreur',
        text:error.message || "Impossible d'ajouter le matériel",
        timer:2500,
    ...
    // composition API pour utiliser $swal
    const { proxy } = getCurrentInstance();

    const notifAjoutMateriel = async () => {
        
        proxy.$swal.fire({
        icon:'success',
        title:'Ajout du matériel ',
        text:"avec succès",
        timer:2500,
        showConfirmButton:false
      })
    ...
    const importerExcel = async (event) =>{
      ...
      const formated = jsonData.map((row) => ({
        ...
        Observation: parseFloat(row["OBSERVATIONS"] || 0)
    ...
    return {
      form,Code, Materiels, Unite, Prix, Observation, Listemateriels,materiels,importerExcel,
      modeEdition, ajouterMateriel, modifierMateriel,
      ...
      search, notifAjoutMateriel,notifModifMateriel
    };
  }
};
</script>
...
.custom-table :deep(.v-data-table__th) {
  background: rgb(67, 81, 174,0.9); /* gris foncé */
  color: white;
  font-weight: 600;
