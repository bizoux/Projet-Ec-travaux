...
export default {
  ...
  methods: {
      ...
      try {
        if (navigator.onLine) {
          ...
          if (error) {
            ...
            // Sauvegarde utilisateur local uniquement si email valide
      ...
      try {
        if (navigator.onLine) {
          ...
          if (error) {
            ...
            if (this.emailSignup && this.passwordSignup) {
              await db.Utilisateur.put({
                email: this.emailSignup,
                password: this.passwordSignup,
                synced: true
          ...
          // Hors ligne : stocke l’utilisateur non synchronisé
          if (this.emailSignup && this.passwordSignup) {
            await db.Utilisateur.put({
              email: this.emailSignup,
              password: this.passwordSignup,
    ...
    async syncOfflineUsers() {
      console.log("🔄 Synchronisation des utilisateurs hors ligne...");

      try {
        let unsyncedUsers = await db.Utilisateur.where("synced").equals(false).toArray();

        // Filtrer uniquement les utilisateurs valides
        unsyncedUsers = unsyncedUsers.filter(
          u => typeof u.email === "string" && u.email.length > 0 && u.password
        );
        ...
        for (const user of unsyncedUsers) {
          try {
            ...
            if (!error) {
              ...
              console.warn("⚠️ Erreur synchro pour", user.email, error.message);
            }
          } catch (err) {
  ...
  async mounted() {
    ...
    const activeSession = await db.Session.toCollection().first();
    if (activeSession) {
    ...
    window.addEventListener("online", async () => {
      this.isOnline = true;
      await this.syncOfflineUsers(); // synchro automatique
    });
    ...
    // Si on démarre déjà en ligne → synchro directe
    if (this.isOnline) {
