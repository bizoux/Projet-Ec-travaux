    ...
    const unsyncedUsers = allUsers.filter(user => user.synced === false);
        ...
        for (const user of unsyncedUsers) {
          try {
            ...
            if (!error) {
              const existing = await db.Utilisateur.get(user.email.trim());
if (existing) {
  await db.Utilisateur.update(user.email.trim(), { synced: true });
} else {
  await db.Utilisateur.put({ email: user.email.trim(), password: user.password, synced: true });
}
