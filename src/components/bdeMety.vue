<template>
  <div class="container my-4">
    ...
    <!-- Tableau BDE -->
    <table class="table table-striped table-bordered table-hover sdp-table" v-if="bdeAvecTravaux.length || selectedDevisId">
      ...
      <tbody>
        ...
        <tr class="table-warning" v-if="selectedDevisId">
          <td></td>
          <td>
          ...
          <td>0</td>
          <td>
            <button class="btn btn-sm btn-primary" @click="ajouterBdeInline">Ajouter</button>
          </td>
        </tr>
    ...
    <div v-else class="styleN">Aucun résultat</div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async ajouterBdeInline() {
      if (!this.selectedDevisId || !this.nouveauTravauxId || !this.nouvelleDesignationId || !this.nouvelleUnite || !this.nouvelleQuantite) {
    ...
    formatQte(val) {
      return val === null || val === undefined || isNaN(val) ? '0,00' : Number(val).toLocaleString('fr-FR', { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    },
    ...
    // Listes
    async ListeProjets() { this.ProjetList = await db.Devis.toArray(); },
