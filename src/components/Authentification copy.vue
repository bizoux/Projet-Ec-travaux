<template>
  <v-container class="fill-height d-flex justify-center align-center">
    ...
    >
      {{ snackbarMessage }}
      <template #actions>
        <v-btn variant="text" color="white" @click="snackbar = false">Fermer</v-btn>
      </template>
    </v-snackbar>
        ...
        for (const user of unsyncedUsers) {
          try {
            ...
            if (!error) {
              ...
              this.snackbarMessage = `Utilisateur ${user.email} synchronisé ✅`;
              this.snackbarColor = "success";
              this.snackbar = true;
              console.log("✅ Synchronisé :", user.email);
            } else {
              if (error.message.includes("already registered")) {
                // Déjà inscrit → on supprime aussi
                await db.Utilisateur.delete(user.email);
                this.snackbarMessage = `Erreur synchro ${user.email}: ${error.message}`;
