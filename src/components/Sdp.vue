<template>
    ...
    <!-- Titre principal -->
      ...
      <div class="container card border-0 shadow mt-4" style="width: 450px;background:linear-gradient(200deg,#cfe4e465,#c6d2df,#a4c3c79a);">
        <div class="card-body text-center">
          <h4 class="mb-3 text-primary border-0 fw-bold" style="font-size: 19px;font-family: 'Trebuchet MS', 'Lucida Sans Unicode', 'Lucida Grande', 'Lucida Sans', Arial, sans-serif;">
            {{ Projet }} <br><hr><span class="text-black" style="font-size: 15px;">CADRE DE SOUS DETAIL DE PRIX</span>
          </h4>
        </div>
      </div>
  ...
  <form @submit.prevent="ajouterTache()">       
      ...
      <div class="bloc-modale" v-if="afficherModaleSdp">
          ...
          <div class="modale card">
              ...
              <h1 class="styleTexte">Ajouter Sdp</h1><br>
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                <div class="mb-3">
                    <label for="" class="form-label">Projet <b style="color: rgb(255, 0, 0);">*</b></label>
                    <input class="form-control" v-model="Projet" disabled required/>
                  </div>
              </div>
              ...
              <div class="col-md-6">
                ...
                <div class="mb-3">
                  ...
                  <label for="" class="form-label">Travaux <b style="color: rgb(255, 0, 0);">*</b></label>
                  
                  <multiselect
                  v-model="travauSelectionner"
                  :options="ListeTravauxProjet"
                  label="nomTravau"
                  track-by="CodeTrav"
                  placeholder="Sélectionnez un travau"
                  @select="UniteCorrespondant"
                />
                </div>

                <!-- <select class="form-control" v-model="travauSelectionner" @change="UniteCorrespondant" required>
        
                  <option disabled value="">-- Choisir une designation --</option>
                  <option v-for="desProjet in ListeTravauxProjet" :key="desProjet.CodeTravau" :value="desProjet">
                    {{ desProjet.CodeTravau }}
                  </option>
  ...
  <div class="row d-flex justify-content-between" >
    ...
    <div class="col-md-2 card shadow-lg border-0 rounded-4 p-4 mb-4" style="font-family: Georgia, 'Times New Roman', Times, serif;background:linear-gradient(200deg, #ffffffb2, #d3e4e8);height: 200px;">
      
      <label for="selectProjet" class="form-label fw-semibold text-black">Travaux du bde</label>
      <multiselect v-model="selectedTache" :options="tacheList" placeholder="Choisir travaux" :searchable="true" :close-on-select="true" :show-labels="false" label="nomTravau" track-by="idTache" @select="chargerTache" class="shadow-sm">
              <template #noResult>
    ...
    <div class="col-md-7 p-2 card border-0 rounded-4">
      ...
      <div v-if="selectedTache">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4 mt-3" style="font-family: 'MaRoboto',sans-serif;">
          
          <h4 class="fw-bold text-primary">Sous Détail de Prix N° {{ selectedTache.Num }}</h4>
          <div>
      ...
      <table class="table table-bordered table-hover sdp-tableDesignation table-sm">
    <thead>
      <tr>
        <th>Désignation</th>
        <th>Unité</th>
  ...
  <!--         
        ...
        <td>
          ...
          <button class="btn btn-sm btn-outline-primary" style="float: right;" v-on:click="RecupererTache(t.idTache,t.nomTravau)"><font-awesome-icon icon="pen" /> Modifier</button>
        </td>
    ...
    <table class="table table-bordered table-hover sdp-table">
      ...
      <div class="bloc-modale" v-if="afficherModaleMaterielAjout">
        ...
        <tr v-for="item in sdp.materiaux" :key="'mat-' + item.idTacheMateriau">
          <td>{{ item.nom }}</td>
          <td>{{ formatQte(item.qteU) }}</td>
          <td>{{ item.uniteJ }}</td>
          <td>{{ formatQte(item.qteTotale) }}</td>
  ...
  <div class="container col-md-3 chatbot-container p-3">
    <!-- Titre -->
    <div class="chatbot-header text-center mb-2">
      <h4 class="fw-bold text-primary mb-0">
        Chatbot <strong style="color: #eb0fe0c4;">BTP</strong> Assistant
      </h4>
    </div>
          ...
          <div class="modale card">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <div class="mb-3">
                    <label for="" class="form-label">Projet <b style="color: rgb(255, 0, 0);">*</b></label>
                    <input class="form-control" v-model="modifProjet" disabled required/>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  ...
                  <multiselect
                  v-model="modifTravau"
                  :options="ListeTravauxProjetModif.map(d => d.Num)"
                  :custom-label="num => ListeTravauxProjetModif.find(d => d.Num === num)?.Travau || ''"
                  placeholder="-- Choisir un travau --"
                  :searchable="true"
                  :allow-empty="false"
                  :close-on-select="true"
                  :show-labels="false"
                  @select="UniteCorrespondantModif"
                />
                  </div>
  ...
  export default {
    ...
    this.idProj = Number(this.$route.query.idProj)

    if (this.idProj) {
      //  récupérer les infos depuis Dexie
      db.Projet.get(this.idProj).then(data => {
        this.projet = data || {}
        this.Projet = data.TacheEffect;
  ...
  const systemPrompt = `
Tu es un professionnel du BTP, style chef de chantier.
Réponds avec des phrases courtes, crédibles et concrètes, comme sur un vrai chantier.
Toujours ce format :

<strong>Quantité par m2:</strong><br>
...
<br>
...
`;
  ...
  try {
    ...
    // 🔍 Vérifie que la réponse contient bien un message
    const botReply = data?.choices?.[0]?.message?.content || "⚠️ Aucune réponse reçue.";
    ...
    console.error("Erreur Edge Function / OpenRouter :", err);
    this.messages.push({
      role: "assistant",
      content: "⚠️ Erreur lors de la communication avec le chatbot.",
    });
  }
},
...
async supprimerMateriel(idTacheMateriel,nom) {
  ...
  async RecupererTache(idTache,nomTravau) {
    ...
    if (tache) {
      ...
      this.modifProjet = tache.idProj;
      this.modifTravau = tache.Num;
  ...
  async RecupererCompPrixMateriel(idTacheMateriel){
    ...
    const existe = await db.taches
      .where({ 
        ...
        Num: this.travauSelectionner.Num // ou la valeur dynamique de ta tâche
      }).first();
    ...
    await db.taches.add({
      ...
      Num:this.travauSelectionner.Num,
      unite:this.unite,
  ...
  async ajouterTacheMateriaux() {
    ...
    const existe = await db.tache_materiaux
      .where({ 
        idTache: this.selectedTache.idTache, 
        idMateriau: this.selectedMateriaux.idMateriau // ou la valeur dynamique de ta tâche
      }).first();
    ...
    await db.tache_materiaux.add({
      idTache: this.selectedTache.idTache,
      idMateriau: this.selectedMateriaux.idMateriau,
      qteU: this.qteUMater,
...
try {
  ...
  this.ListeTravauxProjet = [];
  
  const travauxList = await db.bde
    .where('idProj')
    .equals(Number(this.idProjet))
    .toArray(); 
          // 2. Vider la liste d'abord puis reconstruire
    this.ListeTravauxProjet = await Promise.all(
      travauxList.map(async (trav) => {
        const Trav = await db.Travaux.get(trav.CodeTrav);
        return {
    ...
    console.error("Erreur ListeTravaux :", err);
    this.TravauxList = [];
  }
    ...
    this.ListeDesignationProjet = await Promise.all(
      ...
      async DesignationCorrespondantModif(){
        ...
        if (!this.modifProjet) {
          this.ListeTravauxProjetModif = [];
          return;
        }
  
        const bdeList = await db.bde
          .where("idProj")               // ⚠️ champ clé étrangère vers projet
          .equals(this.modifProjet)
          .toArray();
  
          // 2. Vider la liste d'abord puis reconstruire
    this.ListeTravauxProjetModif = await Promise.all(
      bdeList.map(async (bde) => {
        const trav = await db.Travaux.get(bde.CodeTrav); // FK vers désignation
        return {
          Num: bde.Num,
          Travau: trav.nomTravau,
          Unite:bde.Unite
  ...
  UniteCorrespondant() {
      if (this.travauSelectionner) {
        this.unite = this.travauSelectionner.unite; // 🔹 injection auto
      } else {
        this.unite = "";
      }
    },
  async UniteCorrespondantModif() {
      if (this.modifTravau) {
        const numBde = await db.bde.get(this.modifTravau);
  ...
  async PuCorrespondantMateriel() {
    ...
    async ListeTaches() {
      ...
      for (const tache of tacheList) {
        const bdeItem = await db.bde.get({ Num: tache.Num,idProj:this.idProj })
        if (bdeItem) {
          const trav = await db.Travaux.get({ CodeTrav: bdeItem.CodeTrav })
          if (trav) {
  ...
  async chargerTache(tache) {
      ...
      const tacheCorrespondant = await Promise.all(Taches.map(async t => {
        ...
        if (bdeItem) {
        // On récupère le travau correspondante grâce à CodeTrav
        const trav = await db.Travaux.get({ CodeTrav: bdeItem.CodeTrav });
          return { ...t, nomTravau: trav.nomTravau };
      ...
      const materiaux = await Promise.all(materiauxTaches.map(async t => {
        const materiaux = await db.Materiaux.get(t.idMateriau);
