...
async ajouterBde(indexTravaux, nomTravaux) {
  ...
  if(!this.selectedDevisId){
    alert("Selectionner le projet!");
    return;
  }

  // ✅ Calcul de l’identifiant comme dans ton template : "numRomain-numeroDes"
  const numRomain = indexTravaux + 0;
  ...
  await db.bde.add({
    ...
    Pu: 0,
    Montant: 0,
    codeBde // 🔹 Ici on enregistre le code généré
  });

  alert("Le code bde est "+codeBde);
...
async ajouterBdeNouveau() {
  ...
  // Récupérer les infos du travail et de la désignation
  ...
  // Vérifier si la BDE existe déjà pour ce projet/travail/désignation
  const existe = await db.bde
    .where({
  ...
  // ✅ Générer le code BDE comme "numRomain-numeroDes"
  // numRomain = position du travail dans la liste complète
  const numRomain = this.TravauxList.findIndex(t => t.CodeTrav === this.selectedTraveauId) + 0;

  // numeroDes = nombre d’éléments déjà ajoutés pour ce travail + 1
