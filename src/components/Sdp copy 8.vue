...
<div class="container">
    <br>
  <!-- Titre principal -->
    <div class="card shadow-sm mb-4" style="background-color: #f0f8ffcb;">
...
<div>  
  <label for="selectProjet" class="form-label fw-semibold text-secondary">Désignation</label>
    ...
    <tbody>
      ...
      <tr v-for="item in sdp.mainOeuvre" :key="'mo-' + item.idTacheM">
        ...
        <td>
          ...
          &nbsp;<button class="btn btn-sm btn-outline-danger" v-on:click="supprimerMainOeuvre(item.idTacheM,item.nom)"><font-awesome-icon icon="delete" /> Supprimer</button>
          </center>
        </td>
      </tr>
    ...
    <div class="bloc-modale" v-if="afficherModaleMainOeuvre">
      ...
      <tr v-for="item in sdp.materiels" :key="'mat-' + item.idTacheMateriel">
        ...
        <td>
          ...
          &nbsp;<button class="btn btn-sm btn-outline-danger" v-on:click="supprimerMateriel(item.idTacheMateriel,item.nom)"><font-awesome-icon icon="delete" /> Supprimer</button>
          </center>
...
this.ModifQteTotaleMateriel = val * this.selectedTache.dureeJ;
  ...
  methods: {
    ...
    // SUPPRESSION
      ...
      if (confirm("Etes vous sure de vouloir supprimer "+nom+" ?")) {
...
async modifierTacheMateriel(){
    ...
    await db.tache_materiel.update(this.idTacheMaterielComp, {
      ...
      idTache:this.idTacheMaterielComp, 
