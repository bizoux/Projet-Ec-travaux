          ...
          <tr>
            ...
            <td>
              <multiselect
                ...
                label="Designation"
                track-by="CodeDes"
                @select="modaleDesignation($event.Designation,bde.nomTravaux)"
              >
                <template #noResult>
...
<div class="bloc-modale" v-if="afficherModaleDesignation">
          ...
          <!-- <button type="submit" class="btn btn-primary">Ajouter</button> -->
          <button class="btn btn-sm btn-primary mt-2" @click="ajouterBde(numRomain, DesignationSelectionner)">
                        Ajouter désignation
          </button>
...
export default {
  ...
  methods: {
    ...
    modaleDesignation(nomDesignation,nomTravaux) {
      // if (!item) return;
      this.afficherModaleDesignation = true;
      this.DesignationSelectionner = nomTravaux;

      alert(nomDesignation);
    ...
    async ajouterBde(indexTravaux, nomTravaux) {
      ...
      const numeroDes = (await db.bde.where({ CodeTrav: travail.CodeTrav }).count()) + 0;
