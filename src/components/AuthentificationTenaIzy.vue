<template>
  <v-container-fluid class="fill-height d-flex justify-center align-center fond">
    ...
    <v-card class="pa-7" width="500" elevation="12" rounded="xl">
      <center><img src="../assets/LogoEcTravaux.jpg" alt="" width="80px" style="border-radius: 100%;"></center>
      <!-- Onglets -->
        ...
        <v-window v-model="tab">
          ...
          <v-window-item value="login">
            ...
            <v-form v-model="form" @submit.prevent="loginUser">
              <v-alert v-if="loginError" border="start" type="error" dense class="mb-2">{{ loginError }}</v-alert>
              <v-alert v-if="loginSuccess" border="start" type="success" dense class="mb-2">{{ loginSuccess }}</v-alert>
              ...
              <v-btn type="submit" block color="primary" class="mt-4 mb-3 p-4" :disabled="!form">
                <v-progress-circular
                  v-if="loginLoading"
                  indeterminate
                  color="white"
          ...
          <v-window-item value="signup">
            ...
            <v-form v-model="formInscr" @submit.prevent="registerUser">
              <v-alert v-if="registerError" border="start" type="error" dense class="mb-2">{{ registerError }}</v-alert>
              <v-alert v-if="registerSuccess" border="start" type="success" dense class="mb-2">{{ registerSuccess }}</v-alert>
              ...
              <v-btn type="submit" block color="success" class="mt-4 mb-1 p-4" :disabled="!formInscr">
                <v-progress-circular
                  v-if="signupLoading"
                  indeterminate
                  color="white"
    ...
    methods: {
      ...
      async loginUser() {
        ...
        try {
          ...
          // Vérif format email
          if (!/^\S+@\S+\.\S+$/.test(this.emailLogin)) {
            
            this.loginError = "Le format de l'email est invalide";
      ...
      async registerUser() {
        ...
        // Vérification rapide
        if (!this.emailSignup || !this.passwordSignup || !this.confirmPassword) {
          this.registerError = "Tous les champs sont requis.";
        ...
        if (!/^\S+@\S+\.\S+$/.test(this.emailSignup)) {
          ...
          this.registerError = "Le format de l'email est invalide";
          ...
          for (const user of unsyncedUsers) {
            try {
              ...
              if (!error) {
                ...
                console.warn("Déjà existant :", user.email);
              } else {
...
.v-card {
  background: rgba(255, 255, 255, 255) !important; /* verre transparent */
